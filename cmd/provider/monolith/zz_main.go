...
func main() {
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(clustercontroller.SetupGated_monolith(mgr, clusterOptions), "Cannot setup cluster-scoped AWS controllers")
		kingpin.FatalIfError(namespacedcontroller.SetupGated_monolith(mgr, namespacedOptions), "Cannot setup namespaced AWS controllers")
	} else {
	...
	if setupFn := clustercontroller.NativeSetupHook_monolith; setupFn != nil {
		kingpin.FatalIfError(setupFn(mgr, clusterOptions.Options), "Cannot setup native cluster controllers")
	}
	if setupFn := namespacedcontroller.NativeSetupHook_monolith; setupFn != nil {
