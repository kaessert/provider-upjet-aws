...
func main() {
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(clustercontroller.SetupGated_kafka(mgr, clusterOptions), "Cannot setup cluster-scoped AWS controllers")
		kingpin.FatalIfError(namespacedcontroller.SetupGated_kafka(mgr, namespacedOptions), "Cannot setup namespaced AWS controllers")
	} else {
		logr.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(clustercontroller.Setup_kafka(mgr, clusterOptions), "Cannot setup cluster-scoped AWS controllers")
		kingpin.FatalIfError(namespacedcontroller.Setup_kafka(mgr, namespacedOptions), "Cannot setup namespaced AWS controllers")
	}
	if setupFn := clustercontroller.NativeSetupHook_kafka; setupFn != nil {
		kingpin.FatalIfError(setupFn(mgr, clusterOptions.Options), "Cannot setup native cluster controllers")
	}
	if setupFn := namespacedcontroller.NativeSetupHook_kafka; setupFn != nil {
