...
func main() {
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(clustercontroller.SetupGated_lexmodels(mgr, clusterOptions), "Cannot setup cluster-scoped AWS controllers")
		kingpin.FatalIfError(namespacedcontroller.SetupGated_lexmodels(mgr, namespacedOptions), "Cannot setup namespaced AWS controllers")
	} else {
		logr.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(clustercontroller.Setup_lexmodels(mgr, clusterOptions), "Cannot setup cluster-scoped AWS controllers")
		kingpin.FatalIfError(namespacedcontroller.Setup_lexmodels(mgr, namespacedOptions), "Cannot setup namespaced AWS controllers")
	}
	if setupFn := clustercontroller.NativeSetupHook_lexmodels; setupFn != nil {
		kingpin.FatalIfError(setupFn(mgr, clusterOptions.Options), "Cannot setup native cluster controllers")
	}
	if setupFn := namespacedcontroller.NativeSetupHook_lexmodels; setupFn != nil {
