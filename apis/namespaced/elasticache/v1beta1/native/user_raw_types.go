...
func (src *UserRAW) ConvertTo(dstRaw conversion.Hub) error {
	...
	if len(src.Spec.InitProvider.AuthenticationMode) > 0 {
		...
		dst.Spec.InitProvider.AuthenticationMode = &namespacedv2native.AuthenticationModeRAWInitParameters{
			Type: am.Type,
		}
	} else {
	...
	if len(src.Status.AtProvider.AuthenticationMode) > 0 {
		...
		dst.Status.AtProvider.AuthenticationMode = &clusterv2native.AuthenticationModeRAWObservation{
