...
// Package clusterraw contains the cluster-scoped native controller for
...
import (
	...
	elasticachecluster "github.com/upbound/provider-aws/v2/internal/controller/elasticache/cluster"
	native "github.com/upbound/provider-aws/v2/internal/native"
)

// Setup adds a controller that reconciles ClusterRAW managed resources.
		...
		Complete(managed.NewReconciler(mgr,
			...
			managed.WithTypedExternalConnector[*nativev1beta1.ClusterRAW](
				native.NewTypedConnector(
					...
					func(c *awselasticache.Client, kube client.Client) managed.TypedExternalClient[*nativev1beta1.ClusterRAW] {
						return &typedExternalClient{
							shared: &elasticachecluster.ExternalClient{Client: c, Kube: kube},
						}
					},
				),
			),
...
type typedExternalClient struct {
	shared *elasticachecluster.ExternalClient
}
