...
| Using wrong field type for MaxItems:1 blocks | RAW types must match the TF type's field type **at the same API version**. If TF v1beta2 uses `*Type` (pointer) for a MaxItems:1 block, RAW v1beta2 must also use `*Type`. If TF v1beta1 uses `[]Type` (slice), and the RAW type targets v1beta1, use `[]Type`. Check the TF type at `apis/cluster/<service>/<version>/zz_<resource>_types.go` and mirror the exact Go type. To inspect which blocks are MaxItems:1 in the TF schema: `python3 -c "... block_types ... max_items ..."` (see Section 7). Getting this wrong causes YAML format mismatches — lists vs objects — which break the "mirror TF examples exactly" invariant |
| Modifying committed example manifests to fix schema errors during e2e | If an example manifest fails strict decoding (e.g., `unknown field "spec.forProvider.streamModeDetails[0].streamMode"`), the **type definition** is wrong, not the example. The example was written to mirror the TF example — that is the source of truth. Fix the RAW type to accept the same YAML format as the TF type, regenerate CRDs (`make generate.native`), and re-run. Never change a committed example to match a broken type |
...
| Embedding `xpv2.ManagedResourceSpec` in cluster-scoped types | Never do this — it creates a hybrid interface matching neither `LegacyManaged` nor `ModernManaged`, causing `resolveProviderConfig()` to panic at runtime. Cluster-scoped RAW types must define managed-interface fields explicitly (embed `xpv1.ResourceSpec`) and implement `LegacyManaged` methods in a separate `<resource>_raw_managed.go` file |
| Hardcoding `arn:aws:` prefix in ARN parsing | Never hardcode the `arn:aws:` prefix — it breaks GovCloud (`arn:aws-us-gov:`) and China (`arn:aws-cn:`). Use `strings.Contains(extName, ":<service>:")` or a partition-aware helper like `arn.Parse()` from `github.com/aws/aws-sdk-go-v2/aws/arn` |
| Treating nil slice as "remove all items" or "don't manage" inconsistently | Define nil-vs-empty semantics explicitly for list fields. `nil` = "user didn't set this, don't compare" (skip in `isUpToDate`). `[]T{}` (empty, non-nil) = "user wants zero items." If a user removes a list field entirely (nil), and AWS has items, the controller must either (a) skip the comparison (nil = don't manage) or (b) treat nil as empty and remove items. Document the choice per field. Not deciding causes unrecoverable drift — items enabled in AWS can never be removed by omitting the field |
| `isUpToDate` returns false but `Update` doesn't handle that field value | If `isUpToDate` detects a difference for a field value (e.g., empty string meaning "clear the policy"), the Update path MUST handle that same value. Otherwise the controller enters an infinite reconciliation loop — Observe sees drift, triggers Update, Update does nothing, next Observe still sees drift. Audit every field in `isUpToDate` and verify `Update` covers the same value range |
| Multi-step Update without checking resource state between calls | Some resources require multiple sequential AWS API calls in Update (e.g., Kinesis: UpdateShardCount + UpdateStreamMode + EnableEnhancedMonitoring + ...). If one call transitions the resource to UPDATING/MODIFYING state, subsequent calls fail with `ResourceInUseException`. Either (a) check resource state before each sub-call, or (b) accept that the reconciler will retry on the next cycle. Option (b) works but produces confusing transient errors — document the behavior in code comments |
...
- [ ] Native types registered in `apis/cluster/native_register.go` AND `apis/namespaced/native_register.go` (missing = compiles but fails at runtime)
- [ ] All `+crossplane:generate:reference:extractor=` annotations in **namespaced** types reference `config/namespaced/common.*` extractors (NOT `config/cluster/common.*`)
